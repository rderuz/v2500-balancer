...
st.set_page_config(page_title="V2500 Balancer DSS - Gold Dynamic", layout="wide")
...
def render_fan(data, slot_col, titulo, bolt_w=0, bolt_s=0, fijos=[]):
    ...
    colores = []
    sorted_data = data.sort_values(by=slot_col).reset_index()
...
with st.sidebar:
    ...
    st.divider()
    slots_moviles = st.multiselect("Slots permitidos para optimizar:", options=list(range(1, 23)), default=list(range(1, 23)))
...
if uploaded_file and len(slots_moviles) >= 2:
    ...
    if "last_config_id" not in st.session_state or st.session_state.last_config_id != config_id:
        ...
        for m_name in df_raw['motor'].unique():
            ...
            for i in range(ITERACIONES):
                ...
                if vt < est_min_vib["v"]: est_min_vib = {"v": vt, "moves": mt, "df": combined.copy(), "bolt": bw, "slot": bs}
                if vt <= TOLERANCIA:
                ...
                if vt < (v_ini * 0.6) and mt <= 12:
                    if vt < est_balance["v"]: est_balance = {"v": vt, "moves": mt, "df": combined.copy(), "bolt": bw, "slot": bs}
    ...
    for m_name, datos in st.session_state.resultados_motores.items():
        ...
        seleccionada = st.radio(f"Estrategia:", list(datos["estrategias"].keys()), horizontal=True, key=f"sel_{m_name}")
        res = datos["estrategias"][seleccionada]

        c1, c2, c3, c4 = st.columns(4)
        c1.metric("Vib. Inicial", f"{v_ini:.2f} ips")
        c2.metric("Vib. Final", f"{res['v']:.2f} ips")
        ...
        with g2: st.plotly_chart(render_fan(res['df'], 'nuevo_slot', "PROPUESTA", res['bolt'], res['slot'], fijos=fijos), use_container_width=True, key=f"p2_{m_name}")
        
        # --- TABLA SIN ERRORES DE PANDAS STYLER ---
        df_tab = res['df'][['slot_original', 'peso', 'nuevo_slot']].sort_values(by='slot_original').copy()
        
        def highlight_rows(row):
            if row['slot_original'] in fijos:
                return ['background-color: #f1f2f6'] * len(row)
            elif row['slot_original'] != row['nuevo_slot']:
        ...
        st.table(df_tab.style.apply(highlight_rows, axis=1))
    ...
    st.download_button("📥 DESCARGAR PLAN", data=output.getvalue(), file_name="Plan_V2500_Dynamic.xlsx")
else:

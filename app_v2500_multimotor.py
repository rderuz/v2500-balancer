...
st.set_page_config(page_title="V2500 Balancer DSS - Industrial Pro", layout="wide")
...
def render_fan(data, slot_col, titulo, bolt_w=0, bolt_s=0, fijos=[]):
    ...
    colores = []
    # Asegurar orden visual por slot
    viz_data = data.sort_values(by=slot_col)
...
if uploaded_file and len(slots_moviles) >= 2:
    ...
    if "last_config_id" not in st.session_state or st.session_state.last_config_id != config_id:
        ...
        for m_name in df_raw['motor'].unique():
            ...
            # Memorias
            mems = {k: {"v": v_ini, "moves": 0, "df": df_m.copy(), "bolt": b_ini, "slot": s_ini} 
                   for k in ["vib", "efi", "bal"]}
    ...
    for m_name, datos in st.session_state.resultados_motores.items():
        ...
        sel = st.radio(f"Estrategia:", list(datos["estrategias"].keys()), horizontal=True, key=f"sel_{m_name}")
        ...
        g1, g2 = st.columns(2)
        with g1: st.plotly_chart(render_fan(df_m_ini, 'slot_original', "ACTUAL"), use_container_width=True)
        with g2: st.plotly_chart(render_fan(res['df'], 'nuevo_slot', "PROPUESTA", res['bolt'], res['slot'], fijos), use_container_width=True)
        ...
        # Aplicar estilo seguro para Streamlit
        def style_logic(row):
            if "BLOQUEADO" in row['Acción']: return ['background-color: #f1f2f6'] * len(row)
            if "AL" in row['Acción']: return ['background-color: #f8d7da'] * len(row)
            return ['background-color: #d4edda'] * len(row)

        st.dataframe(df_tab.style.apply(style_logic, axis=1), use_container_width=True)
        
        # Excel
        exp = df_tab.copy()
        exp['Perno_g'] = ""
        exp.loc[exp['nuevo_slot'] == res['slot'], 'Perno_g'] = res['bolt']
        descargas[m_name] = exp
    ...
    with pd.ExcelWriter(output, engine='openpyxl') as writer:
        ...
        for m, d in descargas.items(): d.to_excel(writer, sheet_name=m[:30], index=False)

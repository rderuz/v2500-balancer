ll[df_full['Motor'] == m_name].copy()
            ...
            # Tabla de Taller
            df_tab = ['df'][['ID_Original', 'Peso', 'Momento1', 'Momento2', 'Momento3', 'Slot_Original', 'Nuevo_Slot']].copy()
...
def get_v2500_metrics(df, slot_col):
    if "Vectorial" in metodo_calc:
        res_x, res_y = 0, 0
        # RADIO_CONVERSION: Convierte Desbalance (g-mm) a Peso de Tornillo (g)
        # En el V2500 el alojamiento está a ~165mm
        RADIO_CONVERSION = 165.0 

        for _, row in df.iterrows():
            # Obtener Momento1, si no existe usar Peso * 16.5
        ...
        # Peso del Bolt lógico (Magnitud / Brazo de palanca)
        peso_bolt = round(magnitud_momento / RADIO_CONVERSION, 2)
        # Vibración escala IPS
        ...
        # En modo mitades, el bolt es directamente la diferencia de peso
...
def render_visual_fan(data, slot_col, titulo, bolt_w=0, bolt_s=0):
    ...
    for i in range(22):
        ...
        # Verificar si existe la columna Nuevo_Slot para colorear
        if 'Nuevo_Slot' in data.columns and row['Slot_Original'] != row['Nuevo_Slot']:
            colores.append("#e74c3c") # Rojo: Se mueve
        else:
            colores.append("#2ecc71") # Verde: Se queda
...
if uploaded_file:
    try:
        ...
        for idx, m_name in enumerate(df_full['Motor'].unique()):
            ...
            # Métricas e Información del Bolt
            c1, c2, c3 = st.columns(3)
            v_ini, _, _ = get_v2500_metrics(df_m, 'Slot_Original')
            c1.metric("Vib. Inicial", f"{v_ini:.2f} ips")
            ...
            if choice['v'] > UMBRAL_EXCELENCIA:
                ...
                st.markdown("<div class='excelencia'>🏆 EXCELENCIA: Balance dinámico perfecto.</div>", unsafe_allow_html=True)
            ...
            with g2: st.plotly_chart(render_visual_fan(choice['df'], 'Nuevo_Slot', "AS LEFT", choice['bolt'], choice['slot']), use_container_width=True, key=f"g2_{idx}")
    ...
    st.info("👋 Cargue el Excel para activar el panel visual.")
...
def render_visual_fan(data, slot_col, titulo, bolt_w=0, bolt_s=0):
    ...
    for i in range(22):
        row = data[data[slot_col] == (i+1)].iloc[0]
        fig.add_trace(go.Scatterpolar(r=[6.2], theta=[theta[i]], mode='text', text=[f"<b>{row['ID_Original']}</b>"], textfont=dict(size=11, color="white")))
    ...
    fig.update_layout(title=f"<b>{titulo}</b>", polar=dict(bgcolor='white', angularaxis=dict(tickvals=theta, ticktext=[str(i+1) for i in range(22)], rotation=90, direction="clockwise")),
                      showlegend=False, height=450, margin=dict(t=50, b=30, l=30, r=30))
    return fig
...
if uploaded_file:
    try:
        ...
        for idx, m_name in enumerate(df_full['Motor'].unique()):
            ...
            df_ini_view['Nuevo_Slot'] = df_ini_view['Slot_Original']
            with g1: 
                fig_ini = render_visual_fan(df_ini_view, 'Slot_Original', "SITUACIÓN ACTUAL")
                st.plotly_chart(fig_ini, use_container_width=True, key=f"g1_{idx}")
            with g2: 
                fig_fin = render_visual_fan(choice['df'], 'Nuevo_Slot', "PROPUESTA TALLER", choice['bolt'], choice['slot'])
                st.plotly_chart(fig_fin, use_container_width=True, key=f"g2_{idx}")
